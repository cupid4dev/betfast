...
export const removeExistingSetting = (currentSettings, category, name) => {
  ...
  switch (category) {
    case "rpcNodes":
      updatedSettings.rpcNodes = updatedSettings.rpcNodes.filter(
        (node) => node.name !== name,
      );
      break;
    case "programAddresses":
      updatedSettings.programAddresses =
        updatedSettings.programAddresses.filter(
          (program) => program.name !== name,
        );
      break;
    case "mints":
      updatedSettings.mints = updatedSettings.mints.filter(
        (mint) => mint.name !== name,
      );
      break;
    default:
